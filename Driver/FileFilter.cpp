...
UnloadFilter(IN FLT_FILTER_UNLOAD_FLAGS Flags)
{
	...
	LazyInstance<FileFilter, NonPagedPoolNx>::Dispose();
...
IsDeleteAllowed(PCUNICODE_STRING Filename)
{
	...
	if (sizeof(wszFileName) > Filename->Length)
	{
		RtlCopyMemory(wszFileName, Filename->Buffer, Filename->Length);
	}
	else
	{
		return TRUE;
	...
	std::wstring wstrFileName{ wszFileName };
	auto bFind = rules->IsInProtectDir(wstrFileName);
	...
	PCFLT_RELATED_OBJECTS FltObjects)
{
	...
	ProcessInfo* processInfo = processCtx->FindProcessCtxByPid(ULongToHandle(hPid));
	if (processInfo && processInfo->ProcessPath.size())
	{
		bTrustProcess = processInfo->bTrusted;
	}
	else
	{
		...
		{
			std::wstring wstrProcessPath{ wszProcessPath };
			bTrustProcess = rules->IsTrusProcess(wstrProcessPath);
		}
	}
...
FileFilter::~FileFilter()
{
	...
	if (m_pFileFilter)
	{
		FltUnregisterFilter(m_pFileFilter);
		g_pGlobalData->pFileFilter = nullptr;
		m_pFileFilter = nullptr;
		m_bInitSuccess = FALSE;
	}

}

NTSTATUS FileFilter::Init()
{
	...
	g_pGlobalData->pFileFilter = m_pFileFilter;

	status = FltStartFiltering(m_pFileFilter);
	if (NT_SUCCESS(status))
	{
		m_bInitSuccess = TRUE;
	}
